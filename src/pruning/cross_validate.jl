...
Randomly partition `1:n` into `k` folds of near equal size.
Returns a `Vector` of indices (folds).

TODO: Implement an rng metric / parameter for reproducible folds.
"""
...
"""
	cross_validate(data, target, features, criterion, events, root_error, k=10, minsplit=10, maxdepth=5)

Run k-fold cross validation over the complexity levels generated by `PruneEvent`s

For each fold: fit an independent tree on the k-1 fold, then prune that fold's tree 
to the geometric mean threshold of consecutive alpha pairs.
"""
